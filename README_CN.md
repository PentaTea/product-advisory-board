...
一步到位，同时安装 skill 和全部 9 个 agent 定义。Claude Code 会自动发现插件根目录下默认的 `skills/` 和 `agents/` 目录，因此 skill 和 subagent 会一起可用。
...
**Cursor / Codex 兼容性：** Cursor 2.4+ 原生读取 `.claude/agents/`——以上方式均适用。Codex 使用 TOML 格式的 agent 定义（需格式转换）。

### 2. 在 Claude Code 中触发

如果你是通过 Claude Code Plugin 安装的，请使用带命名空间的 skill 名：
...
安装后，建议先打开 `/agents` 确认 PAB 的 agent 已经被加载：

- Plugin 安装：应看到 `product-advisory-board:pab-scout`、`product-advisory-board:pab-red-team` 以及其他 PAB agents
